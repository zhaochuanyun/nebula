...
struct HttpClient {
  ...
  HttpClient(HostAddr&& h, std::string&& u, std::string&& p) noexcept
      : host(std::move(h)), user(std::move(u)), password(std::move(p)) { connType = "http"; }

  HttpClient(const HostAddr& h, const std::string& u, const std::string& p) noexcept
      : host(h), user(u), password(p) { connType = "http"; }
  ...
  HttpClient(const HostAddr& h, const std::string& u, const std::string& p, const std::string&& c) noexcept
